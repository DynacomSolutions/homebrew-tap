...
Requires a license key to activate. See [AppSwitcher's download page](https://download.apps.dynacom.dev/appswitcher).
...
1. Upload the new build to R2 with a version-specific filename (Homebrew's
   ...
   ```sh
   npx wrangler r2 object put appswitcher-downloads/appswitcher/AppSwitcher-<version>.dmg \
...
3. Optionally also overwrite
   `appswitcher-downloads/appswitcher/AppSwitcher.dmg` as a stable alias
   with the same file.
